...
def load_manifest_entries(manifest_path: Path) -> Dict[str, Dict[str, str]]:
    ...
    with manifest_path.open("r", encoding="utf-8-sig") as f:
        ...
        batch_col = find_column(fieldnames, ["Batch ID", "Batch", "Batch Number", "Batch ", "Batch #"])
...
def main(
    ...
    print(f"Analyzing {len(county_list)} counties: {', '.join(county_list[:5])}{'...' if len(county_list) > 5 else ''}")
        ...
        for (c, imprinted_id), data in ballot_timestamps.items():
            ...
            try:
                sortable.append((
                    ...
                    int(entry["tabulator"]) if entry["tabulator"].isdigit() else entry["tabulator"],
                    int(entry["batch"]) if entry["batch"].isdigit() else entry["batch"],
                    int(entry["position"]) if entry["position"].isdigit() else entry["position"],
                    imprinted_id,
                    data["timestamp"],
                ))
