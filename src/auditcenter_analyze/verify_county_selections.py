...
def load_ballot_manifest(manifest_file: str) -> List[str]:
    ...
    with open(manifest_file, "r", encoding="utf-8-sig") as f:
        ...
        for col in columns:
            ...
            if (col_lower.startswith("#") or col_lower.startswith(".")) and any(
                keyword in col_lower for keyword in ["ballot", "card", "in"]
            ):
    ...
    try:
        with open(contest_file, "r", encoding="utf-8-sig") as f:
            ...
            for row in reader:
                ...
                if (
                    row["audit_reason"] in ["county_wide_contest", "state_wide_contest"]
                    and row["contest_name"] in contests_in_county
                ):
                    targeted.append(
                        {
...
def verify_county(county: str, round_num: int = 3):
    ...
    print("=" * 80)
    print("COUNTY-LEVEL BALLOT SELECTION VERIFICATION")
    ...
    if county not in COUNTY_NAME_TO_ID:
        ...
        print()
        print("To see all valid county names, run:")
    ...
    for contest in targeted:
        print(
            f"  {contest['name']:50s} [{contest['audit_reason']:20s}] ({contest['audited_sample_count']} selections)"
        )
    print()
    ...
    for contest in targeted:
        ...
        if len(counties_with_contest) == 1:
            ...
            print(
                f"Generating {contest['audited_sample_count']} selections for: {contest['name'][:50]}..."
            )
            print(f"  Single-county contest, using {county} manifest ({len(manifest):,} cards)")
            selections = generate_random_numbers(
                SEED, contest["audited_sample_count"], len(manifest)
            )
            ...
            print(
                f"  Combined manifest: {len(combined_manifest):,} cards from {len(counties_with_contest)} counties"
            )
            ...
            all_selections |= selected_ballots
            print(
                f"  ✓ Generated {len(selections)} total selections, {len(selections_in_this_county)} in {county} County"
            )
    ...
    print(
        f"Generated selections (from {len(single_county_contests)} single-county + {len(multi_county_contests)} multi-county contests):"
    )
    ...
    if extra_examined:
        ...
        print("  - Ballots selected for other targeted contests not analyzed here")
        print("  - Manifest ordering or data issues")
    else:
        print("✓ No extra ballots examined beyond generated selections")
        print()

    if not not_examined and not extra_examined:
        print("✓✓✓ PERFECT MATCH: All ballots match exactly!")
        ...
        if extra_examined:
            ...
            print("  These are likely from contests that require CVR data to verify:")
