...
def load_ballot_manifest(manifest_file: str) -> List[str]:
    ...
    with open(manifest_file, "r", encoding="utf-8-sig") as f:
        ...
        # Find columns
        tab_col = next(
            (c for c in columns if any(k in c.lower() for k in ["tabulator", "scanner", "device"])),
            None,
        )
        ...
        count_col = next(
            (
                ...
                and any(k in c.lower() for k in ["ballot", "card", "in"])
            ),
            None,
        )
...
class AuditVerifier:
    ...
    def verify_round(self, round_num: int):
        ...
        self.load_examined_ballots(round_num)
        print()
        ...
        for contest in targeted:
            print(
                f"  {contest['name'][:50]:50s} ({contest['num_counties']:2d} counties, {contest['audited_sample_count']:4d} selections)"
            )
        ...
        for contest in targeted:
            ...
            selections = generate_random_numbers(
                SEED, contest["audited_sample_count"], universe["domain_size"]
            )
            ...
            examined_for_contest = sum(
                1 for b in self.examined_ballots.values() if contest["name"] in b["selected_for"]
            )
        ...
        if not_selected > 0:
            print(
                f"⚠ {not_selected} ballot cards examined but not selected for any targeted contest"
            )
            print("  Possible explanations:")
            print("    - Multi-card ballot packets (Card 1 selected → all cards examined)")
            print("    - Replacement ballots for 'ballot not found'")
            print("    - Phantom ballots")
            print("    - Data errors")
            print()
        ...
        print(
            f"{'County':<15s} {'Examined':>10s} {'Selected':>10s} {'Unaccounted':>12s} {'Status':>10s}"
        )
        print("-" * 80)
        ...
        for county in sorted(by_county.keys()):
            ...
            print(
                f"{county:<15s} {stats['examined']:>10,d} {stats['selected']:>10,d} {stats['unaccounted']:>12,d} {status:>10s}"
            )
        ...
        if not_selected == 0:
            ...
            print(
                f"  All {all_examined:,} examined ballot cards were selected for targeted contests"
            )
        else:
...
def main():
    ...
    if args.all_rounds:
        for round_num in [1, 2, 3]:
            try:
                verifier.verify_round(round_num)
