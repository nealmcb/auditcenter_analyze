...
def main(
    ...
    ballot_data: Dict[str, Dict] = defaultdict(lambda: {
        "county": "",
        "cvr_id": "",
        "timestamps": [],
        "contests": set(),
    ...
    print(f"Multi-timestamp ballots: {len([r for r in single_timestamp_rows if r['is_multi_timestamp']]):,}")
    ...
    print(f"  Ballots with discrepancies: {ballots_with_discrepancies:,} ({100.0 * ballots_with_discrepancies / len(single_timestamp_rows):.1f}%)")
    
    # Time deltas (excluding None values and outliers)
    deltas = [r.get("delta_seconds") for r in single_timestamp_rows if r.get("delta_seconds") is not None]
    if deltas:
        ...
        if reasonable_deltas:
            ...
            print(f"  Min: {min(reasonable_deltas):.1f} seconds ({min(reasonable_deltas) / 60:.1f} minutes)")
            print(f"  Max: {max(reasonable_deltas):.1f} seconds ({max(reasonable_deltas) / 60:.1f} minutes)")
            print(f"  Mean: {sum(reasonable_deltas) / len(reasonable_deltas):.1f} seconds ({sum(reasonable_deltas) / len(reasonable_deltas) / 60:.1f} minutes)")
    ...
    county_stats: Dict[str, Dict] = defaultdict(lambda: {
        "count": 0,
        "total_contests": 0,
    ...
    for county, stats in sorted_counties[:10]:
        ...
        print(f"  {county:20s}: {stats['count']:4d} ballots, "
              f"{avg_contests:.1f} avg contests, "
              f"{stats['total_discrepancies']:3d} discrepancies", end="")
        if avg_delta:
