...
def verify_contest_risk(contest_data: Dict, round_num: int) -> Dict:
    ...
    if status == 'risk_limit_achieved' and not achieved:
        ...
        result['error'] = f"Reported as achieved but calculated risk {calculated_risk:.6f} > {risk_limit}"
...
def verify_round_risks(round_num: int):
    ...
    for r in results[:20]:
        if r.get('n', 0) > 0 and r.get('verified'):
            ...
            print(f"{r['contest'][:50]:<50s} {r['n']:>5d} {r['calculated_risk']:>10.6f} {r['risk_limit']:>10.3f} {status_mark:>10s}")
