...
def show_contest(contest_name=None, county=None, iid=None, data_file=None, discrepancies_only=True):
    ...
    with open(data_file, "r", encoding="utf-8-sig") as f:
        ...
        # Print header
        print(
            f"{'County':15s} {'Contest':40s} {'IID':15s} {'CVR':30s} {'Audit':30s} {'Consensus':10s}"
        )
        print("-" * 160)
        ...
        for row in reader:
            ...
            if match:
                ...
                print(
                    f"{county_short:15s} {contest_short:40s} {iid_short:15s} {cvr:30s} {audit:30s} {row['consensus']:10s}"
                )
                row_count += 1
...
if __name__ == "__main__":
    ...
    if not args:
        ...
        print(
            "  or: python3 show_contest.py [-f|--file path/to/file.csv] ['contest name' | ...] [--all]"
        )
    ...
    if (
        query.startswith("102-")
        or query.startswith("101-")
        or any(
            query.startswith(p)
            for p in [
                "201-",
