...
def by_county(
    ...
    for r in results:
        print(f"{r['county']:<20} {r['n']:>4} {r['outliers_removed']:>8} "
    ...
    for idx, county in enumerate(counties_sorted[: n_rows * n_cols]):
        ...
        if len(set(x_filtered)) < 2:
            ...
            if x_filtered and y_filtered:
                ...
                min_delta = min(y_filtered)
                max_delta = max(y_filtered)
                
                ax.axhline(mean_delta, color="red", linestyle="--", linewidth=1, alpha=0.7, label="Mean")
                ax.text(
                    unique_contest_val,
                    mean_delta,
                    f" {mean_delta:.0f}s",
...
def main(
    ...
    outliers_removed = len(x_contests) - len(x_filtered)
    print(f"Outliers removed: {outliers_removed} ({100.0 * outliers_removed / len(x_contests):.1f}%)")
    ...
    print("=" * 80)
    print(f"Equation: delta_time = {result['intercept']:.2f} + {result['slope']:.4f} * contest_count")
    ...
    print(f"  For a ballot with 30 contests: {result['intercept'] + result['slope'] * 30:.1f} seconds")
    print(f"  For a ballot with 40 contests: {result['intercept'] + result['slope'] * 40:.1f} seconds")

    # Create scatter plot with regression line
    fig, ax = plt.subplots(figsize=(12, 8))
    
    # Scatter plot (filtered data)
    ax.scatter(x_filtered, y_filtered, alpha=0.3, s=20, color="steelblue", label="Data points (outliers removed)")
    
    # Regression line
    x_min, x_max = min(x_filtered), max(x_filtered)
    x_line = [x_min, x_max]
    y_line = [result["intercept"] + result["slope"] * x for x in x_line]
    ax.plot(x_line, y_line, "r-", linewidth=2, label=f"Regression: y = {result['intercept']:.2f} + {result['slope']:.4f}x")
